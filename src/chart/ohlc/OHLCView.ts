...
import OHLCSeriesModel, { OHLCDataItemOption } from './OHLCSeries';
import GlobalModel from '../../model/Global';
import ExtensionAPI from '../../core/ExtensionAPI';
import { StageHandlerProgressParams } from '../../util/types';
import SeriesData from '../../data/SeriesData';
import {OHLCItemLayout} from './ohlcLayout';
...
class OHLCView extends ChartView {

    static readonly type = 'ohlc';
    readonly type = OHLCView.type;
    ...
    render(seriesModel: OHLCSeriesModel, ecModel: GlobalModel, api: ExtensionAPI) {
    ...
    incrementalPrepareRender(seriesModel: OHLCSeriesModel, ecModel: GlobalModel, api: ExtensionAPI) {
        this._clear();
        this._updateDrawMode(seriesModel);
    }
    ...
    _updateDrawMode(seriesModel: OHLCSeriesModel) {
    ...
    _renderNormal(seriesModel: OHLCSeriesModel) {
        ...
        data.diff(oldData)
            .add(function (newIdx) {
                if (data.hasValue(newIdx)) {
                    const itemLayout = data.getItemLayout(newIdx) as OHLCItemLayout;
    ...
    _renderLarge(seriesModel: OHLCSeriesModel) {
        this._clear();
    ...
    _incrementalRenderNormal(params: StageHandlerProgressParams, seriesModel: OHLCSeriesModel) {
        const data = seriesModel.getData();
        const isSimpleBox = data.getLayout('isSimpleBox');

        let dataIndex;
        while ((dataIndex = params.next()) != null) {
            const itemLayout = data.getItemLayout(dataIndex) as OHLCItemLayout;
    ...
    _incrementalRenderLarge(params: StageHandlerProgressParams, seriesModel: OHLCSeriesModel) {
...
function createNormalBox(itemLayout: OHLCItemLayout, dataIndex: number, isInit?: boolean) {
...
function isNormalBoxClipped(clipArea: CoordinateSystemClipArea, itemLayout: OHLCItemLayout) {
    let clipped = true;
...
function setBoxCommon(el: NormalBoxPath, data: SeriesData, dataIndex: number, isSimpleBox?: boolean) {
    const itemModel = data.getItemModel(dataIndex) as Model<OHLCDataItemOption>;
...
function transInit(points: number[][], itemLayout: OHLCItemLayout) {
...
function setLargeStyle(sign: number, el: LargeBoxPath, seriesModel: OHLCSeriesModel, data: SeriesData) {
    // TODO put in visual?
