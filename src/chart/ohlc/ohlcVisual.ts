...
const ohlcVisual: StageHandler = {
    ...
    reset: function (seriesModel: OHLCSeriesModel, ecModel) {

        function getColor(sign: number, model: Model<Pick<OHLCDataItemOption, 'itemStyle'>>) {
            return model.get(
                sign > 0 ? positiveColorQuery : negativeColorQuery
            );
        }

        function getBorderColor(sign: number, model: Model<Pick<OHLCDataItemOption, 'itemStyle'>>) {
            return model.get(
